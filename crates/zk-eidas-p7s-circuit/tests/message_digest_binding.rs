//! messageDigest binding — `blob.message_digest` byte-equals the 32-byte
//! OCTET STRING embedded in `signed_attrs` at the CMS messageDigest
//! attribute position.
//!
//! Closes the soundness gap where an attacker with honest (cert,
//! signed_attrs, sigs) substitutes a fake signed_content whose SHA-256
//! they put into message_digest. Without this binding all other invariants
//! pass independently; this test file proves the gap is closed.
//!
//! Tests:
//!   1. Happy: honest witness round-trips (equality holds).
//!   2. M1 motivating attack: swap signed_content + recompute
//!      message_digest → this binding fails (while content sig + content
//!      digest pass independently). Load-bearing soundness test.
...
//!   5. M4 TSA-inner offset: witnessing offset 930 (the nested
//!      contentTimestamp messageDigest) fails because the inner 32-byte
//!      blob differs from blob.message_digest on the fixture.
...
// v11 blob layout constants — mirror cert_chain_ecdsa.rs / content_signature.rs.
...
// message_digest lives at 1278 (see cert_chain_ecdsa.rs layout comment).
...
#[test]
fn message_digest_binding_happy_round_trips() {
...
/// #31 closes.
#[test]
fn message_digest_binding_motivating_attack_prover_refuses() {
    let inner = build_witness(FIXTURE, b"0x", DUMMY_ROOT_PK).unwrap();
    let w = Witness::new(inner);
...
/// handoff-31 §7.2 M2.
#[test]
fn message_digest_binding_tampered_embedded_digest_prover_refuses() {
    let inner = build_witness(FIXTURE, b"0x", DUMMY_ROOT_PK).unwrap();
    let w = Witness::new(inner);
    let mut honest = w.to_ffi_bytes().expect("serialize");
...
/// `message_digest_binding_tampered_md_offset_bypass_parser` in the
/// `test-bypass-host-anchors` feature gate exercises the in-circuit
/// anchor alone by skipping the host check.
#[test]
fn message_digest_binding_tampered_md_offset_prover_refuses() {
    let inner = build_witness(FIXTURE, b"0x", DUMMY_ROOT_PK).unwrap();
    let w = Witness::new(inner);
    let mut honest = w.to_ffi_bytes().expect("serialize");
...
#[test]
fn message_digest_binding_tsa_inner_offset_prover_refuses() {
    let inner = build_witness(FIXTURE, b"0x", DUMMY_ROOT_PK).unwrap();
    let w = Witness::new(inner);
    let mut honest = w.to_ffi_bytes().expect("serialize");
...
#[test]
fn message_digest_binding_tampered_md_offset_bypass_parser() {
    let inner = build_witness(FIXTURE, b"0x", DUMMY_ROOT_PK).unwrap();
    let w = Witness::new(inner);
    let mut honest = w.to_ffi_bytes().expect("serialize");
