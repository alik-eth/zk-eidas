/* @ts-self-types="./zk-eidas-wasm.d.ts" */
 ...
 * @param {string} known_nullifiers_json
 * @param {string} nullifier_hex
 * @returns {boolean}
 */
export function check_nullifier_duplicate(known_nullifiers_json, nullifier_hex) {
    const ptr0 = passStringToWasm0(known_nullifiers_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passStringToWasm0(nullifier_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.check_nullifier_duplicate(ptr0, len0, ptr1, len1);
    if (ret[2]) {
 ...
 * @returns {string}
 */
export function decode_envelope(cbor_bytes) {
    let deferred3_0;
    let deferred3_1;
    try {
        const ptr0 = passArray8ToWasm0(cbor_bytes, wasm.__wbindgen_malloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.decode_envelope(ptr0, len0);
        var ptr2 = ret[0];
        var len2 = ret[1];
        if (ret[3]) {
            ptr2 = 0; len2 = 0;
 ...
 * @returns {string}
 */
export function parse_proof(proof_json) {
    let deferred3_0;
    let deferred3_1;
    try {
 ...
 * @returns {string}
 */
export function prepare_inputs(sdjwt, claim_name) {
    let deferred4_0;
    let deferred4_1;
    try {
        const ptr0 = passStringToWasm0(sdjwt, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ptr1 = passStringToWasm0(claim_name, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ret = wasm.prepare_inputs(ptr0, len0, ptr1, len1);
        var ptr3 = ret[0];
        var len3 = ret[1];
        if (ret[3]) {
            ptr3 = 0; len3 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('zk-eidas-wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
